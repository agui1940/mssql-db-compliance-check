...
PRINT '';
PRINT '开始检查SQL Server版本......';
IF CAST(SERVERPROPERTY('ProductVersion') AS NVARCHAR(10))NOT LIKE '11.0%'
BEGIN
    PRINT '	***********警告!	SQL版本不是2012!!!';
END;
ELSE
BEGIN
    PRINT '	pass!';
...
BEGIN
    ...
    BEGIN
        PRINT '	***********注意!	数据库:' + @db_name + ' 恢复模式' + @recovery_model
              + '不为完整!!!';
    END;
    SET @int = @int + 1;
END;
